...
assert_contains "network is open mode: no per-subnet forward rules" "<forward mode='open'/>" "${out}"
assert_contains "bridge is named for the connector scan and pinned to the routed zone" "<bridge name='bridge1' zone='libvirt-routed' stp='off' delay='0'/>" "${out}"
